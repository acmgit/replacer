...
minetest.register_privilege("replace", "Player can use the replacer tool") -- register new priv replace to avoid misuse of the tool (Gundul)
...
minetest.register_tool( "replacer:replacer",
{
    ...
    liquids_pointable = false, -- it is ok to painit in/with water
    ...
    on_place = function(itemstack, placer, pointed_thing)
       ...
       if minetest.get_player_privs(name).replace then  -- get information about player privs
	  ...
	  local pos  = minetest.get_pointed_thing_position( pointed_thing, false ); -- Var under is globally and don't exists (dangerous bug)
...
--    on_drop = func(itemstack, dropper, pos),
    ...
    on_use = function(itemstack, user, pointed_thing)
       ...
       return replacer.replace( itemstack, user, pointed_thing, false ); -- Var above is wrong, globally and don't exists (dangerous bug)
    end,
})
